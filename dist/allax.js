...
var Parallax = /** @class */ (function () {
  function Parallax(selector, options) {
    ...
    document.addEventListener('mousemove', function (ev) {
      var _a = _this.options.speedRange,
        min = _a.min,
        max = _a.max;
      var _b = _this.options,
        speed = _b.speed,
        behindMouse = _b.behindMouse,
  ...
  Parallax.prototype.moveLayers = function (ev, currentSpeed, factor, moveX, moveY) {
    for (var i = 0; i < this.layers.length; i++) {
      ...
      this.layers[i].style.transform =
        'translate(' +
        (moveX ? (dataSpeed ? this.position(ev).x * dataSpeed : this.position(ev).x * currentSpeed) : 0) +
        'px, ' +
        (moveY ? (dataSpeed ? -this.position(ev).y * dataSpeed : -this.position(ev).y * currentSpeed) : 0) +
        'px)';
      currentSpeed += factor;
    }
  };
